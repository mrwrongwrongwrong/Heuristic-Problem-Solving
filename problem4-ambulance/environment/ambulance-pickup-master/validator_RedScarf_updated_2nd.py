...
def my_solution(pers, hosps, fname="result.txt"):
    """
    ...
    """
    # Hospitals were selected by clustering
    def clusterHospitals(persons, hospitals, plag=False):
        # personsData = np.array([[person.x, person.y, person.st] for _, person in enumerate(persons)])
        personsData = np.array([[person.x, person.y] for _, person in enumerate(persons)])
        kmeans = KMeans(n_clusters=5, random_state=0).fit(personsData)

        label = kmeans.labels_
        center = kmeans.cluster_centers_
        for index, value in enumerate(center):
            hospitals[index].x = round(value[0])
            hospitals[index].y = round(value[1])

        # visual display
        if plag == True:
            cList = ['r', 'g', 'k', 'b', 'y']
            for index in range(len(personsData)):
                for i in range(len(cList)):
                    if label[index] == i:
                        plt.scatter(personsData[index, 0], personsData[index, 1], s=10, c=cList[i])
            plt.scatter(center[:, 0], center[:, 1], s=40, c='r')
            plt.show()
        return persons, hospitals

    # calculated fitness
    def calculateFitness(solutionnew, persons, hospitals):
        ambulancePath = []
        rescueTime = []
        valuenew = 0
        for index, hospital in enumerate(hospitals):
            for row in range(hospital.num_amb):
                ambulancePath.append([hospital])
                rescueTime.append([0, 1000])

        # add a path
        for indexPerson, value in enumerate(solutionnew):
            # traverse ambulance
            plag = False
            for indexPath in range(len(ambulancePath)):
                ambulance = ambulancePath[indexPath]
                time = rescueTime[indexPath]
                place1 = ambulance[-1]
                place2 = persons[value]
                addTime = take_time(place1, place2) + 1
                # judgment time condition
                if addTime + time[0] + 1 < min(time[1], place2.st):
                    # judgment return to hospital
                    for _, hospital in enumerate(hospitals):
                        allTime = addTime + time[0] + 1 + take_time(place2, hospital)
                        if allTime <= min(time[1], place2.st):
                            ambulancePath[indexPath].append(persons[value])
                            rescueTime[indexPath] = [addTime + time[0], min(time[1], place2.st)]
                            valuenew += 1
                            plag = True
                            break
                if plag == True:
        ...
        for indexPath in range(len(ambulancePath)):
            ...
            # confirm hospital
            for _, hospital in enumerate(hospitals):
                if time[0] + 1 + take_time(place1, hospital) <= time[1]:
                    ambulancePath[indexPath].append(hospital)
                    rescueTime[indexPath] = [time[0] + 1 + take_time(place1, hospital), time[1]]
                    break

        return valuenew, ambulancePath, rescueTime

    persons, hospitals = clusterHospitals(pers, hosps)
    ...
    ambulancePathcurrent = ambulancePathnew.copy()
    rescueTimecurrent = rescueTimenew.copy()
    valuecurrent = 0
    solutionbest = solutionnew.copy()
    ambulancePathbest = ambulancePathnew.copy()
    rescueTimebest = rescueTimenew.copy()
    valuebest = 0

    # iteration of annealing algorithm
    t = temperature[1]
    result = []
    while t > temperature[0]:
        for i in np.arange(markovlen):
            ...
            valuenew, ambulancePathnew, rescueTimenew = calculateFitness(solutionnew, persons, hospitals)
    ...
    with open(fname, "w+") as fil:
        for _, hospital in enumerate(hospitals):
            op = "Hospital:" + str(hospital.x) + "," + str(hospital.y) + "," + str(hospital.num_amb) + "\n"
            fil.write(op)
        op = "\n"
        fil.write(op)
        for _, ambulance in enumerate(ambulancePathbest):
            op = "Ambulance: "
            ambPath = ambulance[0]
            op += str(ambPath.hid) + ": (" + str(ambPath.x) + "," + str(ambPath.y) + ") "
            for indexPath in range(1, len(ambulance)-1):
                op += str(ambulance[indexPath].pid) + ": (" + str(ambulance[indexPath].x) + "," + \
                      str(ambulance[indexPath].y) + "," + str(ambulance[indexPath].st) + "), "
            ambPath = ambulance[-1]
            op += str(ambPath.hid) + ": (" + str(ambPath.x) + "," + str(ambPath.y) + ") \n"
            fil.write(op)
    return
